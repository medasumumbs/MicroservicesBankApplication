...
## *Инструкция по мониторингу при запуске в Docker Compose / Kubernetes*
...
*В системе Prometheus, с которой интегрирован проект, также имплементированы алерты*

Алерт позволяет в случае превышения критического порога конкретной метрики за определенный промежуток времени мгновенно автоматически информировать администраторов проекта о неисправности/угрозе безопасности.

В данном проекте используются следующие алерты:
1) Слишком большое количество неудачных попыток аутентификации в систему (более пяти за последнюю минуту)
2) Более 3 неудачных попыток перевода денежных средств с аккаунта на аккаунт в течение 5 минут.
3) Более двух подозрительных операций от конкретного пользователя за 5 минут
4) Ошибка отправки уведомлений в kafka - хотя бы одна за минуту.
5) Курс валют слишком долго (более минуты) не обновляется
...
В Docker Compose - доступен сразу, в Kubernetes необходимо сначала запустить команду  kubectl port-forward svc/myapp-zipkin 9411:9411 -n default:
...
#### ELK - ElasticSearch, LogStash, Kibana - набор компонент, в совокупности упрощающих мониторинг микросервисных приложений. Необходимы для агрегации логов, их преобразования, хранения, удобного просмотра в пользовательском интерфейсе 

Как реализована интеграция:
1) Каждое из приложений в формате JSON поставляет логи с информацией о трассировании (spanId, traceId), времени, уровне логирования, сервисе-источнике, событии-источнике в топик Kafka с названием logs-<yyyy-mm-dd>
2) Система LogStash (система считывания, преобразования и передачи логов) в качестве consumer с помощью Kafka Input считывает логи по мере их поступления в Kafka
3) LogStash распознает данные в формате JSON, переименовывает некоторые поля, определяет поле @timestamp на основании содержания лога
4) LogStash подключается к ElasticSearch (система хранения и агрегации логов), перекачивает логи по мере их преобразования в базу ElasticSearch
5) ElasticSearch содержит движок по агрегации данных, позволяет выполнять запросы, искать нужные логи по трассировке или сервису, уровню логирования.
6) Для удобства пользователей используется приложение Kibana - своеобразный UI для ELK-стека. Пользователь может настроить в этом приложении нужные визуализации, интеграцию с ElasticSearch и анализировать данные в удобном пользовательском интерфейсе
7) Kibana подключается к ElasticSearch и преобразует данные в удобочитаемый для человека вид.
...
* Перейти в Stack Management
  ![Stack Management](./kibanaScreenshots/stackManagementSettings.png "Stack Management")
* Добавить Data View, при этом нужно указать шаблон наименований индексов ElasticSearch (соответствуют топикам Kafka в данном проекте):
